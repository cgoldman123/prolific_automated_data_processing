        ...
        if task.lower() == 'faces' or task.lower() == 'emotional_faces' or task.lower() == 'emotional faces':
            ...
            for input_dir in input_dirs:
                for x in os.listdir(input_dir):
                    if x.__contains__(subject) and x.__contains__('emotional_faces_v2'):
                        ...
                        missingtrials = np.setdiff1d(list(range(0,200)),pd.Series(data.trial).drop_duplicates().tolist())
                        if len(missingtrials) >0:
                            flags.append(f"Missing trials {missingtrials.tolist()}")
            ...
            for x in os.listdir(input_dir):
                if x.__contains__(subject) and x.__contains__('active_trust'):
                    ...
                    if len(greater_than_27) > 7:
                        flags.append(f'Response patterns on eight or more blocks is more than 90% consistent')
                    sorted_rts = np.sort(rts)
                    if sorted_rts[-5] > 120:
                        flags.append(f"Suspiciously long RTS. Highest 5 RTs are {' '.join(map(str,np.round(sorted_rts[-5:],2)))}")
                    if max(rts) >300:
            ...
            for x in os.listdir(input_dir):
                if x.__contains__(subject) and x.__contains__('blind_dating'):
                    ...
                    missingtrials = np.setdiff1d(list(range(0,108)),pd.Series(data.trial).drop_duplicates().tolist())
                    if len(missingtrials) >0:
                        flags.append(f"Missing trials {missingtrials.tolist()}")
...
# behavioral_checks = task_checks("5c61ab19776b38000156cc95", "coop")
